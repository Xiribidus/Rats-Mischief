...
Elytrats are a special rat type, acquired by equipping rats with the elytrat item, crafted as shown below. Equip/unequip a rat by simply shift-using the rat with the elytrat/bare fist. Normal rats will also equip elytrats upon picking them up from the floor.
...
- **Collection Staff:** Rightclicking a block with this staff will make the rats try to collect that block type, by rightclicking into the air this behavior can be stopped. Rats can break blocks with a hardness of 1 plus 1 per level of strength that is applied to them. Haste effects boost their mining speed by 20% per level of haste.
